...
# 5.7 Fig S7 locomotion and armour proportion barplot
...
# Extract colors
# Palette used: BRBG from color brewer
...
p_locomotion <- ggplot(locomotion_lake_df, aes(fill = locomotion, y = individualCount_species, x = locality)) +
  geom_bar(stat = "identity", position = "fill") +
  labs(fill = "Locomotion", x = "Lake", y = "Proportion") +
...
p_armour <- ggplot(armour_lake_df, aes(fill = material, y = individualCount_species, x = locality)) +
  geom_bar(stat = "identity", position = "fill") +
  labs(fill = "Armour", x = "Lake", y = "Proportion") +
...
# One figure
jpeg(here::here("results","figures","fig_S7_barplot_traits_proportion.jpg"), width = 19, height = 12, units="cm", res=300)

combined_plot <- ggpubr::ggarrange(p_locomotion,p_armour,ncol = 2, nrow = 1, labels = c("A","B"))
combined_plot
